...
fn part_one() -> i32 {
    ...
    while i < input.len() {
        let command = input[i];
        let n: i32 = input[i + 1].parse().unwrap();
        match command {
            "forward" => x += n,
            "down" => depth += n,
            "up" => depth -= n,
