...
public class Day3 {
    ...
    private static boolean lineSegmentsIntersect(LineSegment a, LineSegment b) {
        if (a.vertical == b.vertical) return false;

        return b.vertical && Math.min(a.p1.x, a.p2.x) <= b.p1.x && Math.max(a.p1.x, a.p2.x) >= b.p1.x && a.p1.y <= Math.max(b.p1.y, b.p2.y) && a.p1.y >= Math.min(b.p1.y, b.p2.y) ||
              !b.vertical && Math.min(b.p1.x, b.p2.x) <= a.p1.x && Math.max(b.p1.x, b.p2.x) >= a.p1.x && b.p1.y <= Math.max(a.p1.y, a.p2.y) && b.p1.y >= Math.min(a.p1.y, a.p2.y);
    }

    private static Point getIntersection(LineSegment a, LineSegment b) {
        return a.vertical ? new Point(a.p1.x, b.p1.y) : new Point(b.p1.x, a.p1.y);
    }

    private static List<LineSegment> parseLineSegments(String line) {
        ...
        for (String move : moves) {
            ...
            int newX = x;
            int newY = y;
            int sum = segments.size() == 0 ? length : segments.get(segments.size() - 1).sum + length;
    ...
    static class LineSegment {
        ...
        LineSegment(Point p1, Point p2, boolean vertical, int sum) {
            this.p1 = p1;
            this.p2 = p2;
