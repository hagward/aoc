...
fn main() {
    let input: Vec<Vec<char>> = INPUT
        ...
        .collect();
    let mut adjacent: Vec<Vec<bool>> = vec![vec![false; input[0].len()]; input.len()];
    ...
    for i in 0..input.len() {
        ...
        for j in 0..input[0].len() {
            ...
            if c.is_ascii_digit() {
                ...
                if is_adjacent && number > 0 {
                    numbers.push((i, j_start as usize, j, number));
                    sum += number;
                }
                number = 0;
                is_adjacent = false;
        ...
        if is_adjacent && number > 0 {
            numbers.push((i, j_start as usize, input[0].len(), number));
            sum += number;
        }
    }

    for (i, j_start, j_end, number) in numbers {
        ...
        for j in j_start..j_end {
            ...
            for (i, j) in coords {
                ...
                let i = *i as usize;
                let j = *j as usize;
                if input[i][j] == '*' && !seen.contains(&(i, j)) {
                    if !gears.contains_key(&(i, j)) {
                        gears.insert((i, j), Vec::new());
                    }
                    let number_list = gears.get_mut(&(i, j)).unwrap();
